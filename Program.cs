    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (response == "1")
            {
                ...
                // Use LINQ to find movies whose title contains the search term and return the number and titles
                var MoviesSearch = movieFile.Movies.Where(m => m.title.ToLower().Contains(searchTerm)).Select(m => m.title);
                Console.WriteLine($"There are {MoviesSearch.Count()} movies wtih \"{searchTerm}\" in the title.");
                foreach(var t in MoviesSearch)
                {
                    Console.WriteLine($"  {t}");
                }
            }
